/* ═══════════ js/report.js — shareable weekly report card, drawn as a real PNG ═══════════ */
...
(function(){
  ...
  const QUOTES = [
    'tiny wins compound',
    'done beats perfect',
    'zigzag progress is still progress',
    'your brain did good this week',
    'momentum over perfection',
  ];/* ═══════════ js/report.js — draws the shareable PNG card (used by stats page) ═══════════ */
...
const LoopReport = (() => {
  ...
  async function draw(){
    ...
    const bw = 296, gap = 18, x0 = (W - (bw * 3 + gap * 2)) / 2;
    boxes.forEach((b, i) => {
      const x = x0 + i * (bw + gap), y = 566;
      ctx.fillStyle = b.bg;
      roundRect(ctx, x, y, bw, 150, 18); ctx.fill();
      ctx.strokeStyle = INK; ctx.lineWidth = 5;
      roundRect(ctx, x, y, bw, 150, 18); ctx.stroke();
      ctx.fillStyle = INK;
    ...
    const cw = 64, cgap = 32, cx0 = (W - (cw * 7 + cgap * 6)) / 2, base = 918;
    ...
    s.counts.forEach((n, i) => {
      ...
      roundRect(ctx, x, base - hgt, cw, hgt, 10); ctx.fill();
      ctx.strokeStyle = INK; ctx.lineWidth = 4;
      roundRect(ctx, x, base - hgt, cw, hgt, 10); ctx.stroke();
      ctx.fillStyle = INK;
  ...
  document.getElementById('reportBtn').addEventListener('click', async () => {
    ...
    a.download = `loop-report-${dkey()}.png`;
    a.click();
    btn.textContent = '📊 generate my report card';
